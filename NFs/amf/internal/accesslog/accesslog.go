...
// this NF: the four log files, every timestamp taken to fill them, the httptrace
// callbacks, the request-body UE-id sniffing, the gin inbound middleware, and
// the AMF's per-NAS-message msgtrace.
//
...
//     four 1 MiB buffers are never allocated, and openLog never runs — the log
//     files are never even CREATED. Absence of /tmp/*_log.txt is the acceptance
//     signal for a control build.
...
func Init() {
	...
	// goroutine, no buffers, and crucially no openLog -- so the four log files
	// are never created.
	if !Enabled {
		return
	}

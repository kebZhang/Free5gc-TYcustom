...
func LogNGAP(direction, nasType, ueID string, sctpTime time.Time) {
	...
	b = appendKV(b, "ue_id", ueID, false)
	b = appendKVTime(b, "sctp_time", sctpTime, false)
	b = append(b, '}')
	*bp = b
	enqueue(kindNGAP, bp)
}
...
func LogWorker(ueID, nasType string, start, end time.Time, sbi []SBIView) {
	// Measured worker lines average 519 B and reach 963 B; both the base and
	// the per-SBI increment were short.
	bp := getLine(384+len(sbi)*160)
	b := *bp
	b = append(b, '{')
	b = appendKV(b, "nf", srcNF, true)
	b = appendKV(b, "ue_id", ueID, false)
	b = appendKV(b, "nas_type", nasType, false)
	b = appendKVTime(b, "t_start", start, false)
	b = appendKVTime(b, "t_end", end, false)
	b = appendKey(b, "sbi", false)
	b = append(b, '[')
	for i := range sbi {
		if i > 0 {
			b = append(b, ',')
		...
		b = appendKVTime(b, "before", sbi[i].Before, false)
		b = appendKVTime(b, "after", sbi[i].After, false)
		b = append(b, '}')
	}
	b = append(b, ']')
	b = append(b, '}')
	*bp = b
	enqueue(kindWorker, bp)
}
